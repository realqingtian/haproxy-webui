# HAProxy WebUI 开发计划与进度(M1–M5 归档)

> **归档说明**:本文件是 M1–M5(骨架与部署 → 平台化 → M4 收尾批次)的进度与决策存档,
> 全部里程碑已于 2026-09-17 完成。归档时(2026-09-17)起,进度真源职责移交回根目录 `PLAN.md`,
> 后续迭代按 `docs/ROADMAP.md` 规划开工。仍然长期有效的注意事项已随迁到根 PLAN.md,
> 本文件风险区仅作当时快照,其中第 3、7 条已在文内标注后续状态。
...
3. **dataplaneapi 安全**:5555 端口 = 负载均衡器控制权,只允许 WebUI 后端所在网络访问,严禁公网。
   *(后续状态:2026-09-17 M2 起安全组已放行 5555 公网直连以便开发验证;用户知情接受为临时状态——
   dataplaneapi 有 Basic Auth 且已非 root 运行,但该端口为明文 HTTP,凭据可被链路窃听;转生产前应收紧来源 IP 或加 TLS。)*
...
7. **dataplaneapi 以 root 运行**:它需要重写 haproxy.cfg、执行 systemctl reload、读 root 属主的 runtime socket;M4 考虑普通用户 + polkit/sudo 白名单加固。
   *(后续状态:已于 M5-4 解决——专用用户 dpapi + sudo 白名单(仅 reload/restart haproxy),雨云节点实机回归通过,见 M5 D1 条目与 deploy/dataplaneapi/dataplaneapi.service。)*
