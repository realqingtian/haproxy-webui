...
| 监控 | 单实例 QPS / 连接数 / 流量 / 状态码实时大盘(SSE 实时推送,失败自动回落 10s 轮询);监控总览页聚合全部实例健康与速率,按集群分组,点击下钻;实例监控页可探测节点 Prometheus /metrics 是否可访问并给接入指引 |
...
| Runtime Maps | 在线管理 HAProxy maps(灰度名单 / 分流表):条目增删改即时生效并同步节点文件;支持上传 map 文件(需在配置中引用并 reload 后生效) |
| 日志尾部 | 实例配置 SSH 后实时查看节点 haproxy 日志(SSH tail -f 经 SSE 推送):支持暂停 / 清屏 / 关键字过滤,缓冲上限 2000 行;日志路径可按实例配置(默认 /var/log/haproxy.log) |
...
| 体验细节 | 深色 / 浅色 / 跟随系统主题切换;界面语言中 / 英切换(zh 默认,偏好持久化,全界面覆盖);窄屏移动端可用(抽屉导航 + 响应式布局);大列表分页;前端按路由分包按需加载 |
